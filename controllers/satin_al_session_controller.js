...
exports.sessionSatis = async (req, res) => {
    try {
        if (validate_response(req, res) !== undefined) { return }
        const { information, billingAddress, shippingAddress, buyer, basketItems } = await services.satinAlSessionService.sessionSatis(req)
...
exports.satisKontrol = async (req, res) => {
    try {
        await Checkouts.getFormPayment({
            ...
            if (result?.status == "success") {
                res.json({ message: "Ödemeniz Başarıyla Gerçekleşti", status: "success" })
            }
...
const initializeCheckoutForm = async (information, billingAddress, shippingAddress, buyer, basketItems) => {
    return await Checkouts.initialize({
        ...
        callbackUrl: `http://localhost:${process.env.PORT}/e-ticaret/kitap/session/satis/checkout/complete/payment`, //? Bu url https formatında olmalıdır.
        ...
        Logs.logFile("satislar_session", result)
        console.log(result)
        return result
    }).catch((err) => {
        console.log(err)
        Logs.logFile("satis_hatalari_session", err)
        return err
    })
}
